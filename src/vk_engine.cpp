...
void VulkanEngine::init() {
  ...
  _window = SDL_CreateWindow("Vulkan Mesh Viewer", SDL_WINDOWPOS_UNDEFINED,
...
void VulkanEngine::init_framebuffers() {
  ...
  for (unsigned i = 0; i < swapchain_imagecount; ++i) {
...
void VulkanEngine::init_pipelines() {
  ...
  pipelineBuilder._rasterizer =
      vkinit::rasterization_state_create_info(VK_POLYGON_MODE_LINE);
...
void VulkanEngine::load_meshes() {
  Mesh triMesh{};
  triMesh.load_from_obj("./models/horse_statue_01.obj");

  // load the monkey
  Mesh mainObj{};
  mainObj.load_from_obj("./models/kitchen.obj");
...
void VulkanEngine::init_scene() {
  ...
  mainObj.mesh = get_mesh("mainObj");
  mainObj.material = get_material("defaultmesh");
  mainObj.transformMatrix = glm::mat4{1.0f};

  _numTriangles += mainObj.mesh->_nbTri;

  _renderables.push_back(mainObj);

  _numObj += mainObj.mesh->_nbObj;
}
...
void VulkanEngine::init_descriptors() {
  ...
  for (unsigned short i = 0; i < FRAME_OVERLAP; ++i) {
...
inline void VulkanEngine::set_up_imgui_info(unsigned frameTime,
                                            unsigned lastTime) {
  // imgui new frame
